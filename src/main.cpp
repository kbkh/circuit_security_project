...
int main(int argc, char **argv) {
    ...
    int add_edges = 0;
    //int kmax = 0;
    string outFile = "gnuplotOutput/old/" + outName;
    ofstream kkoutfile(outFile.c_str());
    kkoutfile<<"security"<<"     "<<"# lifted e"<<endl;
    
    string outFileRaw = "gnuplotOutput/old/raw_" + outName;
    ofstream kk2outfile(outFileRaw.c_str());
    kk2outfile<<"security"<<"     "<<"# lifted e"<<endl;
    while(!kdone) {
        ////////////////
        ...
        if ( test_args.size() >= 1 && 4 == atoi(test_args[0].c_str())) {
            ...
            if (!first) {
                kkoutfile<<setfill(' ')<<setw(4)<<security->get_L1()<<setfill(' ')<<setw(15)<<security->get_lifted_edges()<<endl;
                kk2outfile<<setfill(' ')<<setw(4)<<security->get_L1_raw()<<setfill(' ')<<setw(15)<<security->get_lifted_edges_raw()<<endl;
            }
